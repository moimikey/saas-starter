...
export default function Share(props: { url: URL; title: string }) {
  return (
    <div class='flex flex-row my-4 gap-4'>
      ...
      <a
        href={`https://www.linkedin.com/shareArticle?url=${encodeURIComponent(props.url.href)}&title=${
      ...
      <a
        href={`https://reddit.com/submit?url=${encodeURIComponent(props.url.href)}&title=${
          encodeURIComponent(props.title)
        }`}
        target='_blank'
      ...
      <a
        href={`https://twitter.com/share?url=${encodeURIComponent(props.url.href)}&text=${
          encodeURIComponent(props.title)
        }`}
        target='_blank'
